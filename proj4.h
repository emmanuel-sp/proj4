...
/*
 * Calculates each diagonal sum and calls dfs
 */
void calculate(grid * input, grid * output, int inc, int target, int start);

/*
 * Recursively traverses diagonals to find if a diagonal sums to targer
 * Returns true or false.
 */
bool dfs(grid * input, grid * output, int target, int i, int j, int inc);

/**
 * This function is called by both threads to do calculate based on number of threads
 */
void * threadwork(void * args);

/**
 * Used to pass multiple arguments into threadwork
 */
